...
func (ur *UserRepo) GetNicknameByID(ctx context.Context, userID int) (string, error) {
	var username string
	err := ur.SQL.Select(consts.ColumnUsername).
	...
	if err != nil {
		if errors.Is(err, sql.ErrNoRows) {
			return "", RespondWithError(consts.ErrUserNotFound, "user not found")
		}
...
func (ur *UserRepo) GetUserData(ctx context.Context, username string) (*domain.User, error) {
	...
	err := ur.SQL.Select("id", consts.ColumnPasswordHashed).
		From(consts.UsersTable).
		Where(sq.Eq{consts.ColumnUsername: username}).
		RunWith(ur.DB).
		QueryRowContext(ctx).
		Scan(&userID, &hashedPassword)
	if err != nil {
		if errors.Is(err, sql.ErrNoRows) {
			return nil, RespondWithError(consts.ErrUserNotFound, "error getting user data")
		}
...
func (ur *UserRepo) AddUser(ctx context.Context, username string, hashedPassword string) (int, error) {
	...
	err := ur.SQL.Insert(consts.UsersTable).
		Columns(consts.ColumnUsername, consts.ColumnCoins, consts.ColumnPasswordHashed).
...
func (ur *UserRepo) SendCoins(ctx context.Context, transaction *domain.Transaction) error {
	...
	if balance < transaction.Amount {
		return consts.ErrInsufficientFunds
	}

	res, err := ur.SQL.Update(consts.UsersTable).
		Set(consts.ColumnCoins, sq.Case().
	...
	if rowsAffected != 2 {
		return RespondWithError(consts.ErrWrongReceiverID, "error sending coins")
	}
...
func (ur *UserRepo) BuyItem(ctx context.Context, userID int, item string) error {
	...
	err = ur.SQL.Select(consts.ColumnPrice, "id").
		From(consts.MerchTable).
		Where(sq.Eq{consts.ColumnName: item}).
		RunWith(tx).
		QueryRowContext(ctx).
		Scan(&price, &itemID)
	if err != nil {
		if errors.Is(err, sql.ErrNoRows) {
			return RespondWithError(consts.ErrItemNotFound, "error buying item")
		}
	...
	res, err := ur.SQL.Update(consts.UsersTable).
		Set(consts.ColumnCoins, sq.Expr("coins - ?", price)).
		Where(sq.Eq{"id": userID}).
		RunWith(tx).
		ExecContext(ctx)
	if err != nil {
	...
	if rowsAffected == 0 {
		return RespondWithError(consts.ErrUnableToUpdate, "error buying item")
	}
...
func (ur *UserRepo) getUserBalance(ctx context.Context, tx *sql.Tx, userID int) (int, error) {
	query := ur.SQL.Select(consts.ColumnCoins).
		From(consts.UsersTable).
		Where(sq.Eq{"id": userID})
