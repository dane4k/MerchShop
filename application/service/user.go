...
func (us *userService) LoginUser(ctx context.Context, username, password string) (string, error) {
	...
	if userID == 0 {
		...
		if err != nil {
			return "", pgdb.RespondWithError(consts.ErrInternalServerError, "")
		}
...
func (us *userService) SendCoins(ctx context.Context, fromUserID int, request *request.SendCoinRequest) error {
	...
	if err != nil {
		if errors.Is(err, consts.ErrUserNotFound) {
			return consts.ErrWrongReceiver
		}
		return consts.ErrInternalServerError
	}
	...
	if err != nil {
		if errors.Is(err, consts.ErrWrongReceiverID) {
			return consts.ErrWrongReceiver
		}
		if errors.Is(err, consts.ErrInsufficientFunds) {
			return err
		}
		return consts.ErrInternalServerError
	}
...
func (us *userService) BuyItem(ctx context.Context, userID int, request request.BuyItemRequest) error {
	...
	if err != nil {
		if errors.Is(err, consts.ErrItemNotFound) {
			return err
		}
		if errors.Is(err, consts.ErrInsufficientFunds) {
			return err
